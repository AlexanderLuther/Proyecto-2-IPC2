<%-- 
    Document   : agregar-revista
    Created on : 04-oct-2019, 8:09:59
...
<html>    
    ...
    <body class="fondoEditor">
        ...
        <c:if test="${sessionScope.listadoPublicaciones == null}">
            <c:redirect url="/ControladorNuevaRevista"></c:redirect>    
        </c:if>
        <header>
        ...
        <main class="container-fluid">
            <section class="main row">
                ...
                <form class="col-md-4" action="/SistemaWebDeRevistas/ControladorNuevaRevista" enctype="multipart/form-data" method="POST">
                    <center><h2 class="etiquetaBlanca"> Detalles de la Revista </h2></center>
                    <div>
                        <label class="campoObligatorio"> * </label>
                        <label for="idPublicacion" class="etiquetaBlanca">Publicacion</label>   
                        <select class="custom-select custom-select-sm" id="idPublicacion" name="idPublicacion" required> 
                            <option value="" disabled selected> Seleccione una publicacion </option>
                            <c:forEach var="publicacion" items="${sessionScope.listadoPublicaciones}">
                            <option value="${publicacion.idPublicacion}"> ${publicacion.nombrePublicacion} </option>
                            </c:forEach>
                        </select> 
                        <br>
                    </div>
